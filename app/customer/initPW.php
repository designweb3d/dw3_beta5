...
$subject = "Activation du compte et initialisation du mot de passe - " . $CIE_NOM; //je sais pas pk mais on dirait que si je met un accent dans le sujet ca passe pas..
	...
	if ($result2->num_rows > 0) {
        while($row2 = $result2->fetch_assoc()) {
            ...
            <body> 
                ...
                <b>Votre nouveau compte est activé, pour initialiser votre mot de passe, veuillez cliquer sur le lien suivant:</b> <a href="https://' . $_SERVER["SERVER_NAME"] . '/sbin/reinitpw.php?KEY=' . $KEY .'">"https://' . $_SERVER["SERVER_NAME"] . '/sbin/reinitpw.php?KEY=' . $KEY .'"</a><br><hr>Ce lien expire dans 24H ->' . $key_expire . '.' . 
            ...
            /* $sql = "INSERT INTO email (head_from,head_to,to_cc,to_bcc,subject,date_created,box,user_created) VALUES (
                ...
                $sql_task = "INSERT INTO event (event_type,name,description,date_start,customer_id,user_id) 
                    VALUES('EMAIL','Courriel pour initialiser le mot de passe du client','Envoyé par: ".$USER_FULLNAME ."','". $datetime ."','". $ID."','".$USER."')";
