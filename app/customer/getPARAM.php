...
//PARAMETRES CLIENTS
	...
	if ($result->num_rows > 0) {	
		while($row = $result->fetch_assoc()) {
			...
			//CIE
			echo "<option ";
			if ($row["value"] == 'CIE'){ echo "selected ";}
			echo "value='CIE'>Compagnie</option>";
			//nom prenom
			echo "<option ";
			if ($row["value"] == 'FULLNAME'){ echo "selected ";}
			echo "value='FULLNAME'>" . $dw3_lbl["FULLNAME"] . "</option>";
			//nom 
			echo "<option ";
			if ($row["value"] == 'NOM'){ echo "selected ";}
			echo "value='NOM'>" . $dw3_lbl["NOM"] . "</option>";
			//prenom
			echo "<option ";
			if ($row["value"] == 'PRENOM'){ echo "selected ";}
			echo "value='PRENOM'>" . $dw3_lbl["PRENOM"] . "</option>";
			//adresse pays/prov/ville
			echo "<option ";
			if ($row["value"] == 'ADR'){ echo "selected ";}
			echo "value='ADR'>" . $dw3_lbl["ADR"] . "</option>";
			//adresse ligne 1
			echo "<option ";
			if ($row["value"] == 'ADR1'){ echo "selected ";}
			echo "value='ADR1'>" . $dw3_lbl["ADR1"] . "</option>";
			//adresse ligne 2
			echo "<option ";
			if ($row["value"] == 'ADR2'){ echo "selected ";}
			echo "value='ADR2'>" . $dw3_lbl["ADR2"] . "</option>";
			//date creation
			echo "<option ";
			...
			//balance
			echo "<option ";
			if ($row["value"] == 'BALANCE'){ echo "selected ";}
			echo "value='BALANCE'>Balance au compte</option>";
		}
	} else {
			//par defaut
			echo "<option value='ID'>#ID</option>";
			echo "<option value='CIE'>Compagnie</option>";
			...
			echo "<option value='BALANCE'>Balance au compte</option>";
	}
	echo "</select><br>";
	$sql = "SELECT *
			FROM app_prm
	...
	$sql = "SELECT *
			...
			WHERE app_id = '" . $APID . "'
			AND   user_id = '" . $USER . "' 
			AND   name IN ('FULLNAME','ADR','ID','LANG','STAT','TYPE','PRENOM','NOM','CIE','ADR1',
							'ADR2','VILLE','PROV','PAYS','CP','TEL1','TEL2',
							'EML1','EML2','LNG','LAT','NOTE','DTAD','DTMD','BALANCE','FACTURED')
			ORDER BY name";
	$result = $dw3_conn->query($sql);
		...
		while($row = $result->fetch_assoc()) {
			switch ($row["name"]) {
				...
				case 'DTMD':
					...
					break;
				case 'BALANCE':
					$colName = "Balance au compte";
					break;
				case 'FACTURED':
					$colName = "Total des factures impayé";
					break;
			}
			...
			//CIE                               
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_CIE\").click();'>Compagnie : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_CIE' type='checkbox'><span class='slider round'></span></label></td></tr></table>";
			//ADR1                              
			...
			echo "</td><td style='width:60px;'><label class='switch'>       
		     ...
		     //BALANCE                               
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_BALANCE\").click();'>Balance du compte : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_BALANCE' type='checkbox' checked><span class='slider round'></span></label></td></tr></table>";
		     //FACTURED                              
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_BALANCE\").click();'>Total des factures impayés : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_FACTURED' type='checkbox' checked><span class='slider round'></span></label></td></tr></table>";
