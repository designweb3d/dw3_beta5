...
//parametres de l'application 
	...
	$sql = "SELECT *
			...
			WHERE app_id = '" . $APID . "'
			AND   user_id = '" . $USER . "' 
			AND   name IN ('FULLNAME','ADR','ID','LANG','STAT','NOM','ADR1',
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			echo "</td><td style='width:60px;padding-right:20px;'><label class='switch'>
			...
			echo "</td><td style='width:60px;padding-right:20px;'><label class='switch'>       
							<input id='DSP_COL_LANG' type='checkbox'><span class='slider round'></span></label></td></tr></table>";
			//ID                                                            
