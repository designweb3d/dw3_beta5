...
foreach($folder as $file) {
    ...
    if(count($filenames)>=1){
        foreach($filenames as $file) {
            $html .= "<button onclick=\"selLOGO1_IMG('".$target."','".$file."');closeMSG();\" style='margin:0px 3px 0px 3px;color:#111;text-shadow: 1px 1px #1baff3;height:150px;width:150px;background-image: url(\"". $upath . $file . "\");background-size:100% auto;background-repeat:no-repeat;background-position: center;background-color:white;'><span style='background:rgba(255,255,255,0.7);padding:3px;border-radius:3px;margin-top:95px;width:100%;'>" . $file . "</span></button>";
        }
    }
$html .= "</div>";
$dw3_conn->close();
