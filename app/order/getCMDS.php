...
//ORDER BY & WAY
	...
	if ($ORDERBY == "DTLV"){ $ORDERBY = " date_delivery " ;}
	if ($ORDERBY == "DTMD"){ $ORDERBY = " date_modified " ;}
	if ($ORDERBY == ""){ $ORDERBY = " trim(date_modified)" ;}
	if ($ORDERWAY == ""){ $ORDERWAY = " DESC " ;}
	
	
//TOTAL_ROWS

			$sql = "SELECT A.*,  SUBSTRING(`adr1`, LOCATE(' ', `adr1`)) as enSTREET, IFNULL(B.stotal,0) AS enNET
				FROM order_head A
...
//TOTAL_ROWS
		...
		if ($result->num_rows > 0) {	
			...
			<tr style='background:white;'><th></th>";
				...
				if ($DSP_DTLV == "1") { echo "<th>Date livraison</th>"; }
			...
			while($row = $result->fetch_assoc()) {
				...
				if ($DSP_STAT == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>"; 
                    if ($row["stat"] == "0") { echo "<b style='color:green;'>En traitement</b>"; } 
                    else if ($row["stat"] == "1") { echo "<b style='color:blue;'>En facturation</b>"; } 
                    else if ($row["stat"] == "2") { echo "<b style='color:darkred;'>A expédier</b>"; } 
                    else if ($row["stat"] == "3") { echo "<b style='color:orange;'>En expédition</b>"; } 
                    else if ($row["stat"] == "5") { echo "<b style='color:black;'>Terminé</b>"; } 
                    else if ($row["stat"] == "4") { echo "<b style='color:darkgrey;'>Annulé</b>"; } 
                    else if ($row["stat"] == "7") { echo "<b style='color:darkgrey;'>Soumission</b>"; }  echo "</td>";}
				if ($DSP_NOM == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". dw3_decrypt($row["name"]) . "</td>";}
				if ($DSP_CIE == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". $row["company"] . "</td>";}
				if ($DSP_ADR1 == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". dw3_decrypt($row["adr1"]) . "</td>";}
				if ($DSP_ADR2 == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". dw3_decrypt($row["adr2"]) . "</td>";}
				...
				if ($DSP_DTLV == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". $row["date_delivery"] . "</td>";}
				if ($DSP_DTMD == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");'>". $row["date_modified"] . "</td>";}
				if ($DSP_STOT == "1") { echo "<td onclick='getCMD(\"". $row["id"] . "\");' style='text-align:right;'><b>"; if($row["discount"]!= "0"){echo "<span style='color:gold'>**</span>";} echo number_format($row["enNET"],2,'.',',') . "</b>$</td>";}
                echo "</tr>";
			}
			echo "</table></form></div>";

				echo "<div id='hiddenDiv2' style='width:100%;text-align:left;display:inline-block;'>
						<button class='grey' onclick='selALL(\"frmCMD\",\"checkbox\");'><span class='material-icons'>done_all</span></button>";
				echo "<button class='grey' onclick='selNONE(\"frmCMD\",\"checkbox\");'><span class='material-icons'>remove_done</span></button> ";
				if ($APREAD_ONLY == false) { echo " <button class='red' onclick='deleteCMDS();'><span class='material-icons'>delete_sweep</span></button> ";}
				echo " <button onclick=\"ExportToPDF('dataTABLE','Orders');\"><span class='material-icons'>picture_as_pdf</span></button> ";
				echo " <button onclick=\"ExportToExcel('dataTABLE','xlsx','Orders');\"><span class='material-icons'>table_view</span></button> ";
				if ($DSP_STOT == "1") {echo " <b style='float:right;margin-right:5px;'><sup><span style='color:gold'>**</span></sup>Escompté</b>";}
