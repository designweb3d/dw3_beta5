...
$product_qty = $_GET['lgQTE'];
	...
	$sql = "SELECT * FROM product WHERE id = '" .  $prID . "' LIMIT 1";
	$result = mysqli_query($dw3_conn, $sql);
    if ($result->num_rows> 0) {
        $data = mysqli_fetch_assoc($result);
        $line_billing = $data["billing"];
        $qty_min_sold = $data["qty_min_sold"];
        if ($product_qty < $qty_min_sold){
            $product_qty = $qty_min_sold;
        }
    }
...
/*     $date_promo = new DateTime($data["promo_expire"]);
    $now = new DateTime();
    if($date_promo > $now && $line_price > $row["promo_price"]) {
        $line_price = $data["promo_price"];
    } */
...
//verif si escompte produit du fournisseur
/*     $sqlx = "SELECT * FROM customer_discount WHERE customer_id = '".$customer_id."' AND supplier_id = '" . $data["supplier_id"] . "' AND supplier_id <> 0 LIMIT 1";
