...
//parametres de l'application 
	...
	if ($result->num_rows > 0) {	
		while($row = $result->fetch_assoc()) {
			...
			//date modif
			echo "<option ";
			if ($row["value"] == 'TRACKING'){ echo "selected ";}
			echo "value='TRACKING'>Tracking#</option>";
		}
	} else {
			//par defaut
			echo "<option value='ORDER'>Commande #</option>";
			echo "<option value='CIE'>Compagnie</option>";
			echo "<option value='ADR'>" . $dw3_lbl["ORDV"]. "</option>";
			echo "<option selected value='DTCR'>" . $dw3_lbl["DTCR"] . "</option>";
			echo "<option value='TRACKING'>" . $dw3_lbl["TRACKING"] . "</option>";
	}
	echo "</select><br>";
	$sql = "SELECT *
			FROM app_prm
	...
	if ($result->num_rows > 0) {	
		while($row = $result->fetch_assoc()) {
			...
			//par defaut
			echo "<option value='ASC'>" . $dw3_lbl["ORD_ASC"] . "</option>";
			echo "<option selected value='DESC'>" . $dw3_lbl["ORD_DESC"] . "</option>";
	}
	echo "</select></div><br>";
	...
	$sql = "SELECT *
			...
			WHERE app_id = '" . $APID . "'
			AND   user_id = '" . $USER . "' 
			AND   name IN ('ID','STAT','NOM','CIE','ADR1','ORDER',
							'ADR2','VILLE','PROV','PAYS','CP',
							'NOTE','DTAD','DTLV','TRACKING')
			ORDER BY value DESC";
            //echo $sql;
	$result = $dw3_conn->query($sql);
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_CIE\").click();'>Transporteur : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
			...
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_ORDER' type='checkbox' checked><span class='slider round'></span></label></td></tr></table>";
			//ID                               
			...
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_NOTE' type='checkbox'><span class='slider round'></span></label></td></tr></table>";
		     //ID                               
			...
			echo "</td><td style='width:60px;'><label class='switch'>       
		     ...
		     //ID                               
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_DTLV\").click();'>Date Livraison : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_DTLV' type='checkbox'><span class='slider round'></span></label></td></tr></table>";
		     //ID                               
			echo "<table style='width:100%;border:0;'><tr><td width='*'  style='border-bottom:1px dashed #666;cursor:pointer;' onclick='document.getElementById(\"DSP_COL_TRACKING\").click();'>Tracking# : ";
			echo "</td><td style='width:60px;'><label class='switch'>       
							<input id='DSP_COL_TRACKING' type='checkbox' checked><span class='slider round'></span></label></td></tr></table>";                                          
	}                                          
    echo "<br><br></div>";                         
    $dw3_conn->close();
