...
$html="";
			$sql = "SELECT A.*
				FROM user A
				WHERE A.stat = 0 ";
			if ($SS != "")		{ $sql = $sql . " AND  CONCAT(A.id,A.name, A.first_name, A.last_name, A.adr1, A.adr2,A.tel1,A.tel2,A.eml1,A.eml2,A.note) like '%" . $SS . "%' "; }
			$sql = $sql . "
		...
		if ($result->num_rows > 0) {	
			...
			while($row = $result->fetch_assoc()) {
				$html .= "<tr onclick='validateUSR(\"". $row["id"] . "\",\"" . $why . "\");'>";
