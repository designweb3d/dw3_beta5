...
$source_file = basename($_FILES["fileToUpload9"]["name"]);
$imageFileType = strtolower(pathinfo($source_file,PATHINFO_EXTENSION));
$target_file = $target_dir . $_POST['fileName9'] . "." . $imageFileType;
...
// Check file size
if ($_FILES["fileToUpload9"]["size"] > 50000) {
...
// Check if $uploadOk is set to 0 by an error
  ...
  if (move_uploaded_file($_FILES["fileToUpload9"]["tmp_name"], $target_file)) {
    die($_POST['fileName9'] . "." . $imageFileType);
    //die($source_file);
  } else {
