...
if ($cart_act == "" || $cart_act == "invoice"){
    ...
    WHERE A.id = '" . $enID . "' LIMIT 1";
    $result = mysqli_query($dw3_conn, $sql);
    $data = mysqli_fetch_assoc($result);
    $order_id = $data['order_id'];
} else if($cart_act == "order"){
...
if($head_ship_type == "DOM.RP"){
    ...
    <body><h4>Bonjour,</h4>';
        ...
        if ($cart_act == "" || $cart_act == "invoice"){
            if ($head_stat == "1"){
                $mainMessage .= 'La facture #'.$enID.' a été généré! Vérifiez si une expédition ou une autre action est nécessaire<br>';
            } else if ($head_stat == "2"){
                $mainMessage .= 'La facture #'.$enID.' a été payé! Vérifiez si une expédition ou une autre action est nécessaire<br>';
            }
            $sql = "SELECT * FROM invoice_line WHERE head_id = '".$enID."'";
        } else if ($cart_act == "order"){
            $mainMessage .= 'La commande #'.$enID.' a été créé! Vérifiez si une expédition ou une autre action est nécessaire<br>';
          ...
          Détails de la commande:<br><table cellspacing=0 style="width:100%;border-collapse: collapse;">
