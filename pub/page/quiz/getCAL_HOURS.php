...
if ($day_of_week_number == "0"){
    if (intval(substr($CIE_OPEN_J0_H1,0,2)) != 0 && intval(substr($CIE_OPEN_J0_H2,0,2)) != 0){
        for ($i=intval(substr($CIE_OPEN_J0_H1,0,2)); $i < intval(substr($CIE_OPEN_J0_H2,0,2)); $i++) {
            echo "<div onclick=\"updCAL_HOUR('".$year."','".$month."','".$day."','".str_pad($i,2,"0",STR_PAD_LEFT)."','00','".$input_name."')\" class='valid-day' style='cursor:pointer;display:inline-block;padding:10px;border:1px solid #ddd;'>".str_pad($i,2,"0",STR_PAD_LEFT).":00</div>";
            echo "<div onclick=\"updCAL_HOUR('".$year."','".$month."','".$day."','".str_pad($i,2,"0",STR_PAD_LEFT)."','30','".$input_name."')\" class='valid-day' style='cursor:pointer;display:inline-block;padding:10px;border:1px solid #ddd;'>".str_pad($i,2,"0",STR_PAD_LEFT).":30</div>";
            $is_open = true;
        }
    }
}
