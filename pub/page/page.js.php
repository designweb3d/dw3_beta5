...
var KEY = "<?php echo $KEY; ?>";
var LANG = '<?php if(isset($_COOKIE["LANG"])) { echo $_COOKIE["LANG"]; } else if ($USER_LANG != "") { echo $USER_LANG; } else { echo $INDEX_LANG; } ?>';
...
reset_menu_top();
//padding for transparent headers
...
function reset_menu_top(){
    ...
    if (INDEX_HEADER == "/pub/section/header3.php" || INDEX_HEADER == "/pub/section/header4.php" || INDEX_HEADER == "/pub/section/header8.php" || INDEX_HEADER == "/pub/section/header12.php" || INDEX_HEADER == "/pub/section/header14.php" || INDEX_HEADER == "/pub/section/header15.php" || INDEX_HEADER == "/pub/section/header66.php" || INDEX_HEADER == "/pub/section/header17.php" || INDEX_HEADER == "/pub/section/header19.php" || INDEX_HEADER == "/pub/section/header20.php"){
...
function taxeVERTE_INFO() {
    if (USER_LANG == "FR"){
        addMsg("<h2>Que sont les frais environnementaux?</h2><p style='line-height:1;text-align:justify;'>Les frais environnementaux sont des montants recueillis pour financer l'élimination et le recyclage des produits ménagers faisant partie de la réglementation provinciale visant la gestion des déchets. En payant ces frais, vous contribuez à la collecte d'articles, à leur recyclage, à leur réutilisation ou dans les cas où cela n'est pas possible, leur élimination de la façon qui respecte le plus l'environnement. Au Québec, la loi 60 indique que le prix affiché d'un article doit comprendre les frais environnementaux et les frais de reprise. Sur le site "+window.location.host +", les clients du Québec pourront voir les frais environnementaux et les frais de reprise clairement indiqués dans le prix total d'un article.</p><br><br><button onclick='closeMSG();'><span class='material-icons' style='vertical-align:middle;'>done</span>Ok</button>");
    }else{
...
function getEVENT(ID) {
	...
	xmlhttp.onreadystatechange = function() {
	  if (this.readyState == 4 && this.status == 200) {
        ...
        //currentDay=0;
        if(LANG=="FR"){
		    dw3_msg_open("<h3>"+jsonEVENT[0].name_fr+"</h3>"+jsonEVENT[0].description + " <a href='"+jsonEVENT[0].href+"'><button>Plus d'informations</button></a>"+"<br><br><button onclick=\"dw3_msg_close();\">Ok</button>");
        }else{
		    dw3_msg_open("<h3>"+jsonEVENT[0].name_en+"</h3>"+jsonEVENT[0].description_en + " <a href='"+jsonEVENT[0].href+"'><button>More informations</button></a>"+"<br><br><button onclick=\"dw3_msg_close();\">Ok</button>");
        }
        //getDISPO();
	  }
	};
...
//menu
function dw3_tel1() {
  window.open("tel:<?php echo $CIE_TEL1; ?>","_blank");
}
function dw3_tel2() {
  window.open("tel:<?php echo $CIE_TEL2; ?>","_blank");
}
function dw3_eml1() {
    window.open("mailto:<?php echo $CIE_EML1; ?>","_blank");
}
function dw3_eml2() {
    window.open("mailto:<?php echo $CIE_EML2; ?>","_blank");
}
async function dw3_translate(quest,source='fr',target='en',object='dw3_msg') {
    ...
    document.getElementById(object).innerText = await res.json();
    //console.log(await res.json());
}
/* dw3_translate().then((res) => {
    document.getElementById(object).innerText = await res.json();
}); */
...
//forms
function dw3_form_open(form_id,title,url,icon,params){
  if (url=="/pub/section/calendar"){}
  document.getElementById("dw3_form_"+form_id).style.display='inline-block';
}
function dw3_form_close(form_id){
    document.getElementById("dw3_form_"+form_id).style.display='none';
}
//
...
function isInViewport(element,y_margin=50) {
...
function dw3_wish_count(){
    ...
    for(var name in cookies) {
        if(name.slice(0, 5) == "WISH_" && cookies[name] != "0" || name.slice(0, 5) == "WISH2" && cookies[name] != "0"){counter++;}
    }
