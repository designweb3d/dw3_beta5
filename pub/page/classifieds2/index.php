<?php 
//categories d'annonces si cat_lst blank affiche toutes les categories de premier niveau
...
if ($cat_lst !=""){
    ...
    if ($result->num_rows > 0){
        ...
        if($USER_LANG == "FR"){
            echo "<h3 style='background:rgba(0,0,0,0.7);color:white;text-align:left;padding:10px;'><span onclick='history.back()' style='cursor:pointer;'> <span class='material-icons'>skip_previous</span> </span> <span> ".$data["name_fr"]."</span></h3>";
        } else {
            echo "<h3 style='background:rgba(0,0,0,0.7);color:white;text-align:left;padding:10px;'><span onclick='history.back()' style='cursor:pointer;'> <span class='material-icons'>skip_previous</span> <span> ".$data["name_en"]."</span></h3>";
        }
    }
}
...
echo "<div style='max-width:600px;display:inline-block;'><input style='margin:15px 0px 10px 0px;' type='search' class='inputRECH' id='rechAD' oninput=\"findADS(0,TABLES_LIMIT)\"></div><br>";
echo "<div style='min-height:70vh;' id='divCLASSIFIEDS'>";
echo "</div>";

?>
<script>
var cat_lst = '<?php echo $cat_lst; ?>';
TABLES_LIMIT = '10';
