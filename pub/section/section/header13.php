<!-- Transparent minimal 70px Sticky Flex -->
 <?php if (!isset($TABINDEX)){$TABINDEX=0;} ?>
<div style='height:70px;width:100%;'>
    <div id="dw3_head" style='position:fixed;top:0;left:0;display:inline-block;background:transparent;'>
        <table style='width:100%;font-family:var(--dw3_menu_font);border-spacing:0px;border-collapse: collapse;border:0;padding:0;margin-left:auto;margin-right:auto;'>
          <tr style='height:70px;max-height:70px;border:0;padding:0;margin:0;'>
          <?php 
                if (trim($CIE_NOM_HTML)==""){
            ?>      
                <td onclick='dw3_menu_toggle();' style="min-width:110px;text-align:left;border:0;padding:0;margin:0;vertical-align:middle;cursor:pointer;color:var(--dw3_menu_color);font-weight:bold;">
            ...
            <?php 
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        if ($row["target"] == "page" &&  $row["is_in_menu"] == "true"){
                            ...
                            echo "<div style='padding:10px 20px;flex-direction:row;vertical-align:middle!important;text-align:center;' onmouseover=\"dw3_sub_menu_open('dw3_sub".$row["id"]."',1000)\" onmouseout=\"dw3_sub_menu_close('dw3_sub".$row["id"]."',1000)\" onclick=\"dw3_sub_menu_toggle('dw3_sub".$row["id"]."',1000)\"><a style='color:#".$CIE_COLOR9.";' class='no_underline' arial-label='".$title_trad."' tabindex='".$TABINDEX."'>".$title_trad." <span style='font-size:14px;'>&#11206;</span></a><br class='br_small'>";
                            //sub-menu
                ...
                echo "</div></td>";
/*                 $sql = "SELECT * FROM index_head WHERE parent_id=0 AND target='button' AND is_in_menu='true' ORDER BY menu_order ASC";
                $result = $dw3_conn->query($sql);
                if ($result->num_rows > 0) {
                    echo "<td style='text-align:center;'><div style='min-width:250px;display: flex;justify-content: center;' class='dw3_wide_view_menu'>";
                    while($row = $result->fetch_assoc()) {
                        $TABINDEX++;
                        if ($USER_LANG == "FR"){$title_trad = $row["title"];}else{$title_trad = $row["title_en"];}
                        if ($row["target"] == "button" &&  $row["is_in_menu"] == "true"){
                            echo "<button tabindex='".$TABINDEX."' onclick='".$row["url"]."');\" style='white-space: normal;padding:7px 20px;margin:0px 20px;display:inline-block;'> <span class=\"material-icons\" style=\"vertical-align:middle;font-size:30px;text-shadow:".$row["icon_textShadow"].";\">".$row["icon"]."</span> ".$title_trad."</button>";
                        }
                    }
                    echo "</div></td>";
                } */
                ...
                if ($INDEX_WISH=="true"){ $TABINDEX++;
                    ...
                    <td width="40" tabindex='<?php echo $TABINDEX;?>' onclick="dw3_wish_open()" style="max-width:50px;overflow:hidden;vertical-align:middle;text-align:center;cursor:pointer;max-height:50px;height:50px;"> 
                        <div id='dw3_wish_qty' style='position:absolute;display:inline-block;font-family:Arial;font-size:16px;color:#<?php echo $CIE_COLOR8; ?>;width:35px;padding:10px 0px 0px 2.5px;text-align:center;z-index:+1;'><?php echo $dw3_wish_count; ?></div>
            ...
            <td width="40" tabindex='<?php echo $TABINDEX;?>' onclick="dw3_cart_open()" style="max-width:50px;overflow:hidden;vertical-align:middle;text-align:center;cursor:pointer;max-height:50px;height:50px;"> 
                <div id='dw3_cart_qty' style='position:absolute;display:inline-block;font-family:Arial;font-size:16px;color:#<?php echo $CIE_COLOR8; ?>;width:35px;padding:7px 0px 0px 4px;text-align:center;z-index:+1;'><?php echo $dw3_cart_count; ?></div>
                <span class='dw3_font' style='display:inline-block;text-shadow: 1px 1px #222;cursor:pointer;font-size:40px;'>x</span>
            </td>
            <?php } ?>
            <td width="10"> </td>
