...
//info sur la section ACTUELLE 
if ($_SERVER['HTTP_SECTION_IN']??'' !="" && $SID == ""){
    $SID =$_SERVER['HTTP_SECTION_IN'];
}
if (isset($SID) && $SID != ""){
        $sql = "SELECT * FROM index_head WHERE id = '".$SID."';";
        $result = $dw3_conn->query($sql);
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                    $SECTION_PID = $row["parent_id"];
                    $SECTION_HEADER = $row["header_path"];
...
/*         $sql2 ="UPDATE index_head SET total_visited = total_visited + 1 WHERE id = '".$_SERVER['HTTP_SECTION_IN']."'";
