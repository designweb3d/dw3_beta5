<!-- Haut avec entête 102px -> 70px Sticky -->
 <?php if (!isset($TABINDEX)){$TABINDEX=0;} ?>
 <div id="dw3_head21" style="position:fixed;top:0px;left:0px;width:100%;z-index:350;">
    <div style="position:relative;z-index:350;width:100%;line-height:1;color:#<?php echo $CIE_COLOR7; ?>;font-family:<?php echo $CIE_FONT3??'Roboto';?>;text-align:center;max-height:25px;height:29px;overflow:hidden;font-size:16px;padding-top:7px;overflow:hidden;">
        <?php if ($USER_LANG == "FR"){echo $INDEX_TOP_FR;}else{echo $INDEX_TOP_EN;}?>
    </div>
    <div id="dw3_head21bg" style="z-index:-1;position:absolute;top:0px;left:0px;width:100%;height:100px;background:#<?php echo $CIE_COLOR6; if ($CIE_BG5 != ""){echo ";background-image:url('/pub/img/".$CIE_BG5."');background-size: cover;";}  ?>;opacity:0;"></div>
    <div id="dw3_head" style="background:transparent;border-top:<?php echo $CIE_COLOR8_3S; ?>px solid #<?php echo $CIE_COLOR8_3; ?>;border-bottom:<?php echo $CIE_COLOR8_4S; ?>px solid #<?php echo $CIE_COLOR8_4; ?>;">
        <table style='font-family:var(--dw3_menu_font);width:100%;border-spacing:0px;table-layout: auto;border-collapse: collapse;border:0;padding:0;'>
        <tr style='height:70px;max-height:70px;border:0;padding:0;margin:0;'>
            <td onclick='dw3_menu_toggle();' style="min-width:90px;border:0;padding:0;margin:0;text-overflow: ellipsis;vertical-align:middle;text-align:left;cursor:pointer;font-weight:bold;white-space: nowrap;">
            <?php 
                        ...
                        if ($row["target"] == "page" &&  $row["is_in_menu"] == "true"){
                            if (substr($row["url"],0,10)=="/pub/page/" || $row["url"] == "/" || $row["url"] == ""){
                                if ($PAGE_ID == $row["id"] || $PAGE_ID == "0" && $row["url"] == "/" || $PAGE_ID == "" && $row["url"] == "/"){
                                    echo "<td class='dw3_wide_view_menu' style='min-width:100px;vertical-align:middle!important;text-align:center;'><a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' target='_self' style='color:#".$CIE_COLOR9.";text-decoration: underline;text-decoration-color: var(--dw3_selected_border);' arial-label='".$title_trad."' tabindex='".$TABINDEX."'>".$title_trad."</a></td>";
                                } else {
                            ...
                            echo "<td class='dw3_wide_view_menu' style='vertical-align:middle!important;text-align:center;' onmouseover=\"dw3_sub_menu_open('dw3_sub".$row["id"]."',1000)\" onmouseout=\"dw3_sub_menu_close('dw3_sub".$row["id"]."',1000)\" onclick=\"dw3_sub_menu_toggle('dw3_sub".$row["id"]."',1000)\"><a style='color:#".$CIE_COLOR9.";' class='no_underline' arial-label='".$title_trad."' tabindex='".$TABINDEX."'>".$title_trad." <span style='font-size:14px;'>&#11206;</span></a><br class='br_small'>";
                            //sub-menu
            ...
            <td id='dw3_small_view_menu' onclick='dw3_menu_toggle();' style="padding:0px 10px 0px 0px;vertical-align:middle!important;text-align:right;cursor:pointer;">
                <div id='dw3_menu_bars' style='margin-top:7px;'>
