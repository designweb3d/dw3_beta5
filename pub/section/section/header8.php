<!-- Haut avec logo surdimensionné 100px Sticky -->
...
<div style='height:100px;'>
    <div id="dw3_head"  style="border-top:<?php echo $CIE_COLOR8_3S; ?>px solid #<?php echo $CIE_COLOR8_3; ?>;border-bottom:<?php echo $CIE_COLOR8_4S; ?>px solid #<?php echo $CIE_COLOR8_4; ?>;max-height:100px;height:100px;">
        <div id='dw3_h3_tel1' style='<?php if ($CIE_TEL1 == "") { echo "display:none;"; }?>transition: all 1s;position:absolute;top:-2px;left:320px;' class='dw3_wide_view_menu'><span class="material-icons"  style='font-size:19px;color:#<?php echo $CIE_COLOR9; ?>;'>call</span> <a href='tel://<?php echo $CIE_TEL1; ?>' style='font-size:14px;color:#<?php echo $CIE_COLOR9; ?>;font-family:Roboto;'><?php echo $CIE_TEL1; ?></a> </div> 
        <div id='dw3_h3_eml1' style='transition: all 1s;position:absolute;top:-2px;right:50px;' class='dw3_wide_view_menu'><span class="material-icons"  style='font-size:19px;color:#<?php echo $CIE_COLOR9; ?>;'>mail</span> <a href='mailto://<?php echo $CIE_EML1; ?>' style='font-size:14px;color:#<?php echo $CIE_COLOR9; ?>;font-family:Roboto;'><?php echo $CIE_EML1; ?></a> </div>
        <table style='font-family:var(--dw3_menu_font);border-spacing:0px;border-collapse: collapse;border:0;padding:0;'>
          <tr id='dw3_head_row' style='transition: all 1s;height:100px;max-height:100px;border:0;padding:0;margin:0;'>
            <td id='header3_img' onclick='dw3_menu_toggle();' style="min-width:90px;vertical-align:top;text-align:left;cursor:pointer;color:#66c6dd;font-weight:bold;transition: all 1s ease-in-out;">
                <img id="imgLOGO" src="/pub/img/<?php echo $CIE_LOGO3; ?>" style="position:absolute;padding:0px;vertical-align:top;height:auto;width:200px;transition: all 1s;" alt="Logo de l'entreprise pour le site web">
            </td>
                <?php
                $aftermath='';
                //si sous-page voir si il y a dautres sous-pages sinon afficher les pages de la racine PAGE_ID=0
                if ($PAGE_ID != "0" && $PAGE_URL != "/"){
                    $sql = "SELECT * FROM index_head WHERE parent_id='".$PAGE_ID."' AND target='page' OR parent_id='".$PAGE_ID."' AND target='sub' OR parent_id = '".$PAGE_ID."' AND target='section' AND is_in_menu='true' ORDER BY menu_order ASC";
                    ...
                    if ($result->num_rows == 0) {
                        ...
                        $sql = "SELECT * FROM index_head WHERE parent_id='".$PAGE_ID."' AND target='page' OR parent_id='".$PAGE_ID."' AND target='sub' OR parent_id='".$PAGE_ID."' AND target='button' OR parent_id = '".$PAGE_ID."' AND target='section' ORDER BY menu_order ASC";
                        $result = $dw3_conn->query($sql);
                    }
                } else {
                    $sql = "SELECT * FROM index_head WHERE parent_id=0 AND target='page' OR parent_id=0 AND target='sub' OR parent_id=0 AND target='button' OR parent_id = 0 AND target='section' ORDER BY menu_order ASC";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row = $result->fetch_assoc()) {
                            ...
                            if ($row["target"] == "page" &&  $row["is_in_menu"] == "true"){
                                ...
                                echo "<td class='dw3_wide_view_menu' style='min-width:100px;padding-top:5px;vertical-align:middle!important;text-align:center;' onmouseover=\"dw3_sub_menu_open('dw3_sub".$row["id"]."',1000)\" onmouseout=\"dw3_sub_menu_close('dw3_sub".$row["id"]."',1000)\" onclick=\"dw3_sub_menu_toggle('dw3_sub".$row["id"]."',1000)\"><a style='color:#".$CIE_COLOR9.";' class='no_underline' arial-label='".$title_trad."' tabindex='".$TABINDEX."'>".$title_trad." <span style='font-size:14px;'>&#11206;</span></a><br class='br_small'>";
                                //sub-menu
                                ...
                                if ($result2->num_rows > 0) {
                                    ...
                                    while($row2 = $result2->fetch_assoc()) {
                                        ...
                                        if (substr($row2["url"],0,10)=="/pub/page/"){
                                            echo "<button tabindex='".$TABINDEX."' onclick=\"window.open('".$row2["url"]."?PID=".$row2["id"]."&P1=".$row2["cat_list"]."','_self');\" style='white-space: normal;padding:10px;margin:1px;margin:1px 0px;width:97%;max-width:350px;display:inline-block;' onfocus=\"dw3_sub_menu_open('dw3_sub".$row["id"]."',1000)\" onblur=\"dw3_sub_menu_close('dw3_sub".$row["id"]."',1000)\">".$title_trad."</button><br class='br_tiny'>";
                                        }else{
...
/*                                     $aftermath .= "\n 
                ...
                <?php } ?><td style="min-width:10px;"> </td>
            <td width="40" id='dw3_small_view_menu' onclick='dw3_menu_toggle();' style="padding-top:10px;vertical-align:middle!important;text-align:center;cursor:pointer;">
