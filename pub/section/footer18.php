...
<footer>
    <?php if (!isset($dw3_conn)){header('Location: https://'.$_SERVER["SERVER_NAME"].'/');exit;}?>
        <div class='dw3_page_foot' style='padding-top:10px;'>
        <div style="z-index:+1;position:absolute;right:0px;font-size:0.8em;display:inline-block;height:2vw;min-height:50px;overflow:hidden;text-align:right;">
        <?php 
            ...
            echo "</div>";
        if ($CIE_LOGO5 != ""){
            echo "<div style='vertical-align:top;display:inline-block;margin:20px;'>";
                if ($CIE_LOGO5 == "1"){
                    if($USER_LANG == "FR"){ 
                    ...
                    echo "<img src='/pub/img/".  $CIE_LOGO5 . "' width='230' style='margin:10px;'><br>";
                }
                echo "</div>";
            }
            ?>

        <div class='dw3_paragraf' style='line-height:1.4;vertical-align:top;display:inline-block;width:auto;min-width:250px;margin-top:20px;text-align:left;'>
            <h3 style='text-align:left;margin-bottom:5px;'><?php if($USER_LANG == "FR"){ echo "En savoir plus"; }else{echo "More infos";} ?></h3> 
                <?php 
                    echo "&nbsp;&nbsp;".$CIE_EML1 . "<br>&nbsp;&nbsp;". $CIE_TEL1 ."<br>&nbsp;&nbsp;". $_SERVER["SERVER_NAME"]."";
                ?>
        </div>
        <div class='dw3_paragraf' style='line-height:1.4;vertical-align:top;display:inline-block;width:auto;min-width:250px;margin-top:20px;text-align:left;'>
            <h3 style='text-align:left;margin-bottom:5px;'>Navigation</h3>
            <?php 
                ...
                if ($dw3_result->num_rows > 0) {
                    while($row = $dw3_result->fetch_assoc()) {
                        if ($row["is_in_menu"] == "true"){
                            if ($row["target"] == "page"){
                                if($USER_LANG == "FR"){
                                    echo "&nbsp;<a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."'>&nbsp;".$row["title"]."</a><br class='br_small'>";
                                }else{
                                    echo "&nbsp;<a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."'>&nbsp;".$row["title_en"]."</a><br class='br_small'>";
                                }
                            }
                        }
                    }
        ...
        <div class='dw3_paragraf' style='line-height:1.4;display:inline-block;vertical-align:top;min-width:250px;margin-top:20px;width:auto;text-align:left;'>
        ...
        if ($CIE_COOKIES_IMG == ""){ 
            ...
            echo "<br><img alt='Image pour les cookies' title='"; 
            if($USER_LANG == "FR"){ 
                echo "Préférences en matière de conservation des données"; 
            }else{
                echo "Data retention preferences";
            }
            echo "' style='float:left;cursor:pointer;width:auto;height:2vw;min-height:40px;z-index:+1;' src='/pub/img/cookies/". $CIE_COOKIES_IMG."' onclick='dw3_cookie_pref();'>";
        }
        ?>
