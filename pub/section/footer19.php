...
<footer>
    <?php if (!isset($dw3_conn)){header('Location: https://'.$_SERVER["SERVER_NAME"].'/');exit;}?>
        ...
        <div style='display:inline-block;margin-bottom:50px;'>
            <div class='dw3_wide_view_parent' style='max-width:1200px;'>
                <div class='dw3_paragraf dw3_wide_view_div' style='width:50vw;max-width:600px;padding-top:40px;margin:5px 20px;line-height:1.4em; text-shadow: 1px 1px 2px #222;color:#<?php echo $CIE_COLOR7; ?>;'>
                    <h1 style='margin:0px 0px 20px 0px;text-align:left;'>CONTACT</h1>
                    TÉLÉPHONE<br><?php echo $CIE_TEL1; ?>
                    <?php 
                    $dw3_sql = "SELECT * FROM location ORDER BY name ASC";
                    $dw3_result = $dw3_conn->query($dw3_sql);
                        if ($dw3_result->num_rows > 0) {
                            while($row = $dw3_result->fetch_assoc()) {
                                    echo  "<div style='margin-bottom:20px;'> </div>".$row["name"]."<br>".$row["adr1"]." ".$row["adr2"]." ".$row["city"].", ".$row["province"]." ".$row["postal_code"]."</a>";
                            }
                        }
                    ?>
                </div>
                <div class='dw3_high_view_menu' style='width:50vw;max-width:600px;width:auto;padding-top:40px;margin:5px 20px;text-align:left;line-height:2;'>
                            ...
                            if ($dw3_result->num_rows > 0) {
                                while($row = $dw3_result->fetch_assoc()) {
                                    if ($row["is_in_menu"] == "true"){
                                        if ($row["target"] == "page"){
                                            if($USER_LANG == "FR"){
                                                echo "<a class='hovera' href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' style='color:#".$CIE_COLOR7."; text-shadow: 1px 1px 2px #222;text-align:center;text-transform: capitalize;'>".$row["title"]."</a><br class='br_small'>";
                                            }else{
                                                echo "<a class='hovera' href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' style='color:#".$CIE_COLOR7."; text-shadow: 1px 1px 2px #222;text-align:center;text-transform: capitalize;'>".$row["title_en"]."</a><br class='br_small'>";
                                            }
                                        }
                                    }
                                }
        ...
        if ($CIE_COOKIES_IMG == ""){ 
            ...
            echo "<div style='text-align:left;margin-bottom:-50px;'><img alt='Image pour les cookies' title='"; 
            if($USER_LANG == "FR"){ 
                echo "Préférences en matière de conservation des données"; 
            }else{
                echo "Data retention preferences";
            }
            echo "' style='cursor:pointer;margin:5px;width:auto;height:2vw;min-height:40px;z-index:+1;' src='/pub/img/cookies/". $CIE_COOKIES_IMG."' onclick='dw3_cookie_pref();'></div>";
        }
        ?>
        <div style="font-size:1em;padding:10px 0px;width:100%;max-width:100%;overflow:hidden;display:inline-block;text-align:center;">
        ...
        <div style='font-size:0.9em;padding:3px;'>
            <?php if($USER_LANG == "FR"){ echo "Créé avec"; }else{echo "Created with";} ?><a href='https://dw3.ca' target='dw3'> <u>Design Web 3D</u></a>
        </div>
