<!-- Haut avec entête 102px -> 70px Sticky FLEX -->
<?php if (!isset($TABINDEX)){$TABINDEX=0;} ?>
<div id="dw3_head22" style="position:fixed;top:0px;left:0px;width:100%;z-index:350;">
    <div id="dw3_head22bg" style="z-index:-1;position:absolute;top:0px;left:0px;width:100%;height:70px;background:#<?php echo $CIE_COLOR8; if ($CIE_BG5 != ""){echo ";background-image:url('/pub/img/".$CIE_BG5."');background-size: cover;";}  ?>;opacity:0;"></div>
                        ...
                        if ($row["target"] == "page" &&  $row["is_in_menu"] == "true"){
                            if (substr($row["url"],0,10)=="/pub/page/" || $row["url"] == "/" || $row["url"] == ""){
                                if ($PAGE_ID == $row["id"] || $PAGE_ID == "0" && $row["url"] == "/" || $PAGE_ID == "" && $row["url"] == "/"){
                                    echo "<div style='padding:10px 20px;flex-direction:row;vertical-align:middle!important;text-align:center;'><a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' target='_self' style='color:#".$CIE_COLOR9.";text-decoration: underline;text-decoration-color: var(--dw3_selected_border);' arial-label='".$title_trad."' tabindex='".$TABINDEX."'>".$title_trad."</a></div>";
                                } else {
