...
<footer>
    <?php if (!isset($dw3_conn)){header('Location: https://'.$_SERVER["SERVER_NAME"].'/');exit;}?>
        <div class='dw3_page_foot' style='min-height:0px;'>
                ...
                if ($dw3_result->num_rows > 0) {
                    while($row = $dw3_result->fetch_assoc()) {
                        if ($row["is_in_menu"] == "true"){
                            if ($row["target"] == "page"){
                                if($USER_LANG == "FR"){
                                    echo "<div style='display:inline-block;padding:10px 15px;'><a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' style='text-align:center;'>".$row["title"]."</a></div>";
                                }else{
                                    echo "<div style='display:inline-block;padding:10px 15px;'><a href='".$row["url"]."?PID=".$row["id"]."&P1=".$row["cat_list"]."' style='text-align:center;'>".$row["title_en"]."</a></div>";
                                }
                            }
                        }
                    }
        ...
        if (trim($CIE_STRIPE_KEY) != "" || trim($MAG_INTERAC??'false') != "false"){
            echo "<div style='width:100%;display:inline-block;height:40px;text-align:left;'>";
            echo "<img alt='Image de cartes de crédit' src='/pub/img/dw3/cc.png' style='height:40px;width:40px;margin:10px;'>";
            echo "<img alt='Image de Interact' src='/pub/img/dw3/interac.png' style='height:40px;width:40px;margin:10px;'>";
            echo "</div><br>";
        }
        if ($CIE_LOGO5 != ""){
            if ($CIE_LOGO5 == "1"){
        ...
        if ($CIE_COOKIES_IMG == ""){ 
            echo "<br><button onclick='dw3_cookie_pref();'>";
                if($USER_LANG == "FR"){ 
        ...
        ?>
        <div style="padding:5px 0px;width:100%;max-width:100%;overflow:hidden;display:inline-block;text-align:center;">
        ...
        <div style='font-size:1.2em;overflow:hidden;width:100%;padding:13px 0px 10px 0px;background-color:#<?php echo $CIE_COLOR6; ?>;color:#<?php echo $CIE_COLOR7; ?>;'>
