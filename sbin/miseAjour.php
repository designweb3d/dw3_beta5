...
$datetime2 = date("Y-m-d_H.i.s");
$driver = new mysqli_driver();
$driver->report_mode = MYSQLI_REPORT_ALL & ~MYSQLI_REPORT_INDEX;
//ini_set('memory_limit', '256M'); 
//require_once $current_path . '/sbin/security.php'; 
...
$DW3_VERSION = file_get_contents($root_path . '/VERSION');
...
if ($DW3_VERSION == $DW3_UPDATE_VERSION){
    $dw3_conn->close();
    die("La mise à jour a déja été complétée.");
}
...
$update_file = fopen($root_path . '/backup/update_'.$datetime2.'.txt', "w");

    //header
    //header('Content-Type: text/octet-stream');
    //header('Cache-Control: no-cache');
    $dw3_source_path = "https://ww7.ca/dw3/".$DW3_UPDATE_VERSION;
    $dw3_target_path = $root_path;
    ...
    //flush();
    //sleep(1);
    $update_txt = "Début de la mise à jour: " . date('l, F j, Y') . " " .date("h:i:sa") . "\n\n";
...
/*     if ($_SERVER["SERVER_NAME"] != "designweb3d.com" 
    ...
    && $_SERVER["SERVER_NAME"] != "kymcocanada.com" 
    ...
    && $_SERVER["SERVER_NAME"] != "demo.ww7.ca"){ */
        $dw3_file = fopen($root_path."/VERSION", "w");
...
if ($DW3_UPDATE_VERSION == "BETA5"){
    require_once $current_path . '/dw3_func.php';
...
$sql = "INSERT INTO event (event_type,name,description,date_start) 
        VALUES('SYSTEM','Tâche planifié - Mise à jour','/sbin/update_".$datetime2.".txt','". $datetime ."')";
$result = $dw3_conn->query($sql); 

//echo "\n"."Nb de fichiers MAJ: " . $dw3_upd_count ."/".max((int)$dw3_upd_list_count, 0). "\n";
